...
public class ClienteServ {
	...
	public void validarRegistro(Cliente miCliente) {
		...
		if (miCliente.getIdCliente() > 0) {
			...
			JOptionPane.showMessageDialog(null,"El codigo de cliente no puede ser negativo","Advertencia",JOptionPane.WARNING_MESSAGE);
	...
	public void validarModificacion(Cliente miCliente) {
		ClienteDao miClienteDao;
		if (miCliente.getNombreCliente().length()<250) {
			miClienteDao = new ClienteDao();
			miClienteDao.modificarCliente(miCliente);	
			modificaCliente=true;
		}else{
			JOptionPane.showMessageDialog(null,"El nombre de la Cliente debe ser menor a 250 digitos","Advertencia",JOptionPane.WARNING_MESSAGE);
